...
test.describe("App Loading", () => {
  ...
  test("should load required JavaScript files", async ({ page }) => {
    ...
    const flutterLoaded = loadedScripts.some((url) => url.includes("flutter"));
    expect(flutterLoaded, "Flutter framework should be loaded").toBeTruthy();

    // Verify no critical scripts failed to load
    expect(
      failedScripts,
      "No JavaScript files should fail to load",
    ).toHaveLength(0);
  });

  test("should have accessible content for screen readers", async ({
    page,
  }) => {
    // Navigate to the app
    await page.goto("http://127.0.0.1:8080/", { waitUntil: "networkidle" });
