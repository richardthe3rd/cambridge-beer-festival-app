...
test.describe("URL Routing - Basic Routes (Phase 1 - Festival-Scoped)", () => {
  ...
  test("should navigate to festival home without errors", async ({ page }) => {
    await page.goto(`http://127.0.0.1:8080/${defaultFestivalId}`, {
  ...
  test("should navigate to festival-scoped favorites route", async ({
    page,
  }) => {
    await page.goto(`http://127.0.0.1:8080/${defaultFestivalId}/favorites`, {
      waitUntil: "networkidle",
    });
    await waitForPageReady(page);
    ...
    expect(page.url()).toBe(
      `http://127.0.0.1:8080/${defaultFestivalId}/favorites`,
    );
  });

  test("should navigate to global about route (no festival scope)", async ({
    page,
  }) => {
  ...
  test("should navigate to festival info route", async ({ page }) => {
    await page.goto(`http://127.0.0.1:8080/${defaultFestivalId}/info`, {
      waitUntil: "networkidle",
    });
    await waitForPageReady(page);
...
test.describe("Deep Linking - Parameterized Routes (Phase 1 - Festival-Scoped)", () => {
  test("should handle deep link to drink route", async ({ page }) => {
    ...
    await page.goto(
      `http://127.0.0.1:8080/${defaultFestivalId}/drink/${category}/${drinkId}`,
      { waitUntil: "networkidle" },
    );
    await waitForPageReady(page);
    ...
    expect(page.url()).toContain(
      `/${defaultFestivalId}/drink/${category}/${drinkId}`,
    );
  });
  ...
  test("should handle deep link to brewery route", async ({ page }) => {
    ...
    await page.goto(
      `http://127.0.0.1:8080/${defaultFestivalId}/brewery/${breweryId}`,
      { waitUntil: "networkidle" },
    );
    await waitForPageReady(page);
  ...
  test("should handle URL-encoded style names with lowercase canonical format", async ({
    page,
  }) => {
    // Test with lowercase canonical URL (app generates lowercase URLs)
    const style = "american ipa";
    await page.goto(
      `http://127.0.0.1:8080/${defaultFestivalId}/style/${encodeURIComponent(style)}`,
      { waitUntil: "networkidle" },
    );
    await waitForPageReady(page);
  ...
  test("should redirect invalid festival ID to default festival", async ({
    page,
  }) => {
    // Navigate with invalid festival ID
    await page.goto("http://127.0.0.1:8080/invalid-fest", {
      waitUntil: "networkidle",
    });
    await waitForPageReady(page);
  ...
  test("should preserve query parameters when redirecting invalid festival", async ({
    page,
  }) => {
    // Navigate with invalid festival ID and query params
    await page.goto(
      "http://127.0.0.1:8080/invalid-fest?search=IPA&category=beer",
      { waitUntil: "networkidle" },
    );
    await waitForPageReady(page);
...
test.describe("Browser Navigation (Phase 1 - Festival-Scoped)", () => {
  ...
  test("should maintain history across multiple navigations", async ({
    page,
  }) => {
...
test.describe("Page Refresh (Phase 1 - Festival-Scoped)", () => {
  test("should preserve festival-scoped route on page refresh", async ({
    page,
  }) => {
    // Navigate to favorites
    await page.goto(`http://127.0.0.1:8080/${defaultFestivalId}/favorites`, {
  ...
  test("should preserve query parameters on page refresh", async ({ page }) => {
    ...
    await page.goto(
      `http://127.0.0.1:8080/${defaultFestivalId}?search=IPA&category=beer`,
      { waitUntil: "networkidle" },
    );
    await waitForPageReady(page);
