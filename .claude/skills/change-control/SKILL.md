...
| Claim about Dart/Flutter syntax or type behavior | Check the "Review-Comment Defence Facts" list in AGENTS.md (const exception constructors, `TlsException` subtyping, conditional-import stubs stay out of barrels, null-aware elements, `break`-free switch cases) — and skill `reference` for the glossary. The Copilot-facing copy is `.github/skills/code-review-dart/SKILL.md`. |
