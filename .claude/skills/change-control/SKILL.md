...
| `analysis_options.yaml` | Never, except the `dart_code_linter:` key | The lint baseline is the enforced house style; loosening it invalidates the "CI is ground truth" gate, and tightening it creates repo-wide churn. Change requires a deliberate maintainer decision. **Carve-out:** the `dart_code_linter:` block is the only supported place to configure DCL rules (hardcoded config filename, no CLI override) and is inert to `flutter analyze`, so it does not touch the lint baseline. Editing it still requires measuring the rule's hits first — the block records each rejected rule with its count and reason. |
...
| Claim about Dart/Flutter syntax or type behavior | Check the "Review-Comment Defence Facts" list in AGENTS.md (const exception constructors, `TlsException` subtyping, conditional-import stubs stay out of barrels, null-aware elements, `break`-free switch cases) — and skill `reference` for the glossary. The Copilot-facing copy is `.github/skills/code-review/SKILL.md`. |
