...
| `analyze` job | New analyzer warnings/errors | `./bin/mise run analyze lib/<changed-dir>/` then grep `ANALYZE_LOG` | Fix the findings; never edit `analysis_options.yaml` (Do-Not-Modify list — its `dart_code_linter:` key is the sole carve-out and is inert to `flutter analyze`, so it cannot fix an analyze failure) | — |
