...
async function checkPage(url, options = {}) {
  ...
  try {
      ...
      try {
        const hasFlutterContent = await page.evaluate(() => {
          ...
          return (
            ...
            body.querySelector("flt-glass-pane, flutter-view, [flt-renderer]")
          );
        });
    ...
    if (!flutterInitialized) {
      console.log(
        `Warning: Flutter app may not have fully initialized after ${maxWaitTime}ms`,
      );
    }
  ...
  if (errors.length > 0) {
    ...
    errors.forEach((err, idx) => {
      ...
      if (err.location) {
        console.log(
          `   Location: ${err.location.url}:${err.location.lineNumber}:${err.location.columnNumber}`,
        );
      }
      if (err.stack) {
  ...
  if (warnings.length > 0) {
    ...
    warnings.forEach((warn, idx) => {
      ...
      if (warn.location) {
        console.log(
          `   Location: ${warn.location.url}:${warn.location.lineNumber}:${warn.location.columnNumber}`,
        );
      }
    });
  }
